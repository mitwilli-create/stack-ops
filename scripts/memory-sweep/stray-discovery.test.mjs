...
test('discovery rejects a permissive or symlinked ledger without reading its target', () => {
  const f = fixture();
  try {
    transcript(project(f, 'canonical'), 'eligible-id');
    chmodSync(f.ledgerPath, 0o644);
    assert.throws(() => discoverCandidates({
      projectsRoot: f.projectsRoot,
      ledgerPath: f.ledgerPath,
      vaultRoot: f.vaultRoot,
      quiescenceMinutes: 45,
    }), /exact private regular file \(0600\)/i);

    const sentinel = join(f.root, 'sentinel-ledger.txt');
    writeFileSync(sentinel, 'eligible-id\n', { mode: 0o600 });
    rmSync(f.ledgerPath);
    symlinkSync(sentinel, f.ledgerPath);
    assert.throws(() => discoverCandidates({
      projectsRoot: f.projectsRoot,
      ledgerPath: f.ledgerPath,
      vaultRoot: f.vaultRoot,
