...
import { appendFileSync, chmodSync, existsSync, lstatSync, mkdtempSync, mkdirSync, readFileSync, readdirSync, realpathSync, writeFileSync, symlinkSync, utimesSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import test from 'node:test';
import { fileURLToPath } from 'node:url';
import * as strayDrainModule from './stray-drain.mjs';
...
const SELECTOR_ID_ONE = 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa';
const SELECTOR_ID_TWO = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb';
...
function preparePrivateLogDirectory(f) {
  const parent = join(f.root, 'logs');
  const logDir = join(parent, 'stray-drain');
...
test('coordinator arguments are explicit, bounded, and unambiguous', () => {
  assert.deepEqual(strayDrainModule.parseCoordinatorArgs([], 20), {
    dryRun: false,
    idsFile: null,
    requestedLimit: 20,
  });
  assert.deepEqual(strayDrainModule.parseCoordinatorArgs(['--dry-run', '--limit', '7'], 20), {
    dryRun: true,
    idsFile: null,
    requestedLimit: 7,
  });
  assert.deepEqual(strayDrainModule.parseCoordinatorArgs(['--ids-file', '/private/requested-ids'], 20), {
    dryRun: false,
    idsFile: '/private/requested-ids',
    requestedLimit: 20,
  });
});

test('coordinator arguments reject unknown, positional, duplicate, incomplete, and conflicting input', () => {
  for (const argv of [
    ['--bogus'],
    ['positional'],
    ['--dry-run', '--dry-run'],
    ['--limit', '2', '--limit', '3'],
    ['--ids-file', '/one', '--ids-file', '/two'],
    ['--limit'],
    ['--ids-file'],
    ['--limit', '2', '--ids-file', '/one'],
    ['--limit', '0'],
    ['--limit', '21'],
  ]) {
    assert.throws(() => strayDrainModule.parseCoordinatorArgs(argv, 20), /argument|limit|conflict/i);
  }
});

test('requested identifier files require private stable canonical identifiers', () => {
  const root = mkdtempSync(join(tmpdir(), 'stack-stray-selector-'));
  const selector = join(root, 'ids.txt');
  writeFileSync(selector, `${SELECTOR_ID_TWO}\n${SELECTOR_ID_ONE}\n`, { mode: 0o600 });
  assert.deepEqual(strayDrainModule.readRequestedIds(selector, 2), [SELECTOR_ID_TWO, SELECTOR_ID_ONE]);

  chmodSync(selector, 0o644);
  assert.throws(() => strayDrainModule.readRequestedIds(selector, 2), /private regular file/i);
  chmodSync(selector, 0o600);
  writeFileSync(selector, `${SELECTOR_ID_ONE}\n${SELECTOR_ID_ONE}\n`, { mode: 0o600 });
  assert.throws(() => strayDrainModule.readRequestedIds(selector, 2), /duplicate/i);
  writeFileSync(selector, `${SELECTOR_ID_ONE.toUpperCase()}\n`, { mode: 0o600 });
  assert.throws(() => strayDrainModule.readRequestedIds(selector, 2), /canonical transcript identifier/i);

  const target = join(root, 'target.txt');
  const link = join(root, 'link.txt');
  writeFileSync(target, `${SELECTOR_ID_ONE}\n`, { mode: 0o600 });
  symlinkSync(target, link);
  assert.throws(() => strayDrainModule.readRequestedIds(link, 2), /private regular file/i);
});

test('requested candidates preserve request order and fail on missing or ambiguous discovery', () => {
  const candidates = [
    { id: SELECTOR_ID_ONE, path: '/safe/one.jsonl', mtimeMs: 1, size: 10 },
    { id: SELECTOR_ID_TWO, path: '/safe/two.jsonl', mtimeMs: 2, size: 20 },
  ];
  assert.deepEqual(
    strayDrainModule.selectRequestedCandidates(candidates, [SELECTOR_ID_TWO, SELECTOR_ID_ONE]),
    [candidates[1], candidates[0]],
  );
  assert.throws(
    () => strayDrainModule.selectRequestedCandidates(candidates, ['33333333-3333-4333-8333-333333333333']),
    /not currently eligible/i,
  );
  assert.throws(
    () => strayDrainModule.selectRequestedCandidates([...candidates, { ...candidates[0] }], [SELECTOR_ID_ONE]),
    /ambiguous/i,
  );
});

test('requested selection refuses identifier or candidate drift during startup recovery', () => {
  const beforeIds = [SELECTOR_ID_ONE];
  ...
  assert.doesNotThrow(() => strayDrainModule.assertRequestedSelectionStable(
    beforeIds,
    [...beforeIds],
    beforeCandidates,
    beforeCandidates.map((row) => ({ ...row })),
  ));
  assert.throws(() => strayDrainModule.assertRequestedSelectionStable(
    beforeIds,
    [SELECTOR_ID_TWO],
    beforeCandidates,
    beforeCandidates,
  ), /identifier file changed/i);
  assert.throws(() => strayDrainModule.assertRequestedSelectionStable(
    beforeIds,
    beforeIds,
    beforeCandidates,
    [{ ...beforeCandidates[0], size: 11 }],
  ), /membership changed/i);
});
...
test('mined record provenance requires a private untracked non-symlink vault path', () => {
  ...
  assert.throws(() => validateMinedRows(rows, vault, untracked), /exact private regular file \(0600\)/i);
});
...
test('rollback resumes after a crash that moved a record before publishing its manifest', async () => {
  ...
  mkdirSync(join(quarantineRoot, 'project-memory'), { mode: 0o700 });
  mkdirSync(join(quarantineRoot, 'project-memory', 'documents'), { mode: 0o700 });
  mkdirSync(join(quarantineRoot, 'project-memory', 'documents', 'sessions'), { mode: 0o700 });
...
test('unknown coordinator arguments fail before default discovery or child execution', () => {
  const f = fixture();
  transcript(join(f.project, `${SELECTOR_ID_ONE}.jsonl`));
  const env = wrapperEnvironment(f, `
    import { writeFileSync } from 'node:fs';
    writeFileSync(process.env.FORBIDDEN_DRainer_MARKER, 'invoked');
  `);
  const marker = join(f.root, 'drainer-invoked');
  env.FORBIDDEN_DRainer_MARKER = marker;
  const logDir = preparePrivateLogDirectory(f);
...
test('identifier selector invokes the child with only the exact requested eligible transcript', () => {
  const f = fixture();
  const requested = join(f.project, `${SELECTOR_ID_ONE}.jsonl`);
  const newer = join(f.project, `${SELECTOR_ID_TWO}.jsonl`);
  transcript(requested, 180);
  transcript(newer, 120);
  const selector = join(f.root, 'requested-ids.txt');
  writeFileSync(selector, `${SELECTOR_ID_ONE}\n`, { mode: 0o600 });
  const env = wrapperEnvironment(f, `
    import { readFileSync, writeFileSync } from 'node:fs';
    const args = process.argv.slice(2);
    const list = args[args.indexOf('--list') + 1];
    writeFileSync(process.env.SELECTOR_RECEIPT, readFileSync(list));
    process.exit(7);
  `);
  const receipt = join(f.root, 'selected-source.txt');
  env.SELECTOR_RECEIPT = receipt;
  preparePrivateLogDirectory(f);
  const result = spawnSync(process.execPath, [WRAPPER, '--ids-file', selector], { env, encoding: 'utf8' });
  assert.notEqual(result.status, 0);
  assert.equal(readFileSync(receipt, 'utf8'), `${realpathSync(requested)}\n`);
});
